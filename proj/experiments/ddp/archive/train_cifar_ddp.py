...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            pred = output.argmax(dim=1, keepdim=True)  # get class (index) of prediction
...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='PyTorch DDP with CIFAR10 on ResNet18')
    ...
    trainset = datasets.CIFAR10('./datasets',
                              train=True,
                              download=True,
                              transform=transform)
    testset = datasets.CIFAR10('./datasets',
                              train=False,
                              download=True,
    ...
    model = Net().to(local_rank) # models.resnet18(num_classes=10).to(local_rank)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_epoch(args, ddp_model, local_rank, train_loader, optimizer, epoch)
